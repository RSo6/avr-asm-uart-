...
**Figure 2.3 — UCSRC register format**
...
**Figure 2.4 — UCSRB register format**
...
**Figure 2.5 — Device operation algorithm flowchart**

The flowchart shows the following sequence:
1. Start
2. Initialization of the UART interface
3. Configure Port C for signal transmission
4. Receive character from virtual terminal
5. Output received character "Q" to LEDs
6. Configure Port A to receive analog signal from keys
7. Read key states into register R16
8. Output received key signal to LEDs
9. Output key state to oscilloscope and terminal
10. End
...
**Figure 3.13 — Transmission of the number: 90(hex) = 10010000(bin)**
...
To determine the achieved transmission speed, which in this example was set at 4800 bits/sec = 4800 symbols/sec = 4800 baud, the oscilloscope signal sweep is configured to align the sides of the pulse with the vertical grid lines of the oscilloscope (Figure 3.15).

**Figure 3.15 — Determining the achieved transmission speed on the oscilloscope**

As seen in the figure, the duration of one pulse is approximately 0.104 ms. If we divide one bit by this time and multiply by 1000, we obtain the specified speed:

V = (1 / 208.33) × 1000 = 4800 bit/sec
